...
import { DollarSign, TrendingUp, Calendar, ArrowUpRight, Filter, Download, Activity, PieChart as PieIcon, BarChart3, LineChart as LineIcon, ShieldCheck } from 'lucide-react';
...
export function AdminReports() {
  ...
  if (loading) return (
    <div className="h-[70vh] flex flex-col items-center justify-center text-slate-500 font-medium">
    ...
    <div className="space-y-8 p-4 md:p-8 -m-4 md:-m-8 bg-slate-50 min-h-screen">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        <div>
          ...
          <p className="text-sm font-bold text-slate-500 mt-1">Real-time revenue metrics and property operations.</p>
        </div>
        <div className="flex gap-3">
          <Button className="bg-white hover:bg-slate-100 text-slate-900 border border-slate-200 rounded-[1rem] h-12 px-6 shadow-sm font-bold transition-all"><Filter className="w-4 h-4 mr-2" /> Filter Data</Button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm flex flex-col justify-between group hover:border-indigo-200 transition-colors">
          <div className="flex justify-between items-start mb-6">
            <div className="p-3 bg-indigo-50 text-indigo-600 rounded-[1.25rem]"><DollarSign className="w-6 h-6" /></div>
        ...
        <div className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm flex flex-col justify-between group hover:border-emerald-200 transition-colors">
          <div className="flex justify-between items-start mb-6">
            <div className="p-3 bg-emerald-50 text-emerald-600 rounded-[1.25rem]"><TrendingUp className="w-6 h-6" /></div>
          ...
          <div>
            <p className="text-slate-400 text-[10px] font-black uppercase tracking-widest mb-1">Avg. Booking Value</p>
        ...
        <div className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm flex flex-col justify-between group hover:border-blue-200 transition-colors">
          <div className="flex justify-between items-start mb-6">
            <div className="p-3 bg-blue-50 text-blue-600 rounded-[1.25rem]"><Calendar className="w-6 h-6" /></div>
        ...
        <div className="p-6 md:p-8 border-b border-slate-100 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-slate-500 text-xs font-bold mt-0.5">Recent payment settlements and status transitions.</p>
            </div>
          </div>
            ...
            <tbody className="divide-y divide-slate-50 text-sm">
              {[
                ...
                <tr key={i} className="hover:bg-slate-50/50 transition-colors group">
                  <td className="px-8 py-5 text-slate-400 font-medium">{l.t}</td>
                  <td className="px-8 py-5 text-slate-900">{l.r}</td>
                  <td className="px-8 py-5 text-slate-500 uppercase text-xs">{l.m}</td>
                  <td className="px-8 py-5 text-slate-900 text-base">{(l.a)}</td>
